...
```{r utility-functions}
# Utility functions are now loaded from config.R
# aspix_x() and aspix_y() are available globally
```
...
d <- readChar(CONTACTS_DATA_FILE, nchars=(COL_SHOE*ROW_SHOE+2)*NUM_SHOE)
data <- list()
for(i in 1:NUM_SHOE)
{
...
acciden<-read.csv(LOCATIONS_DATA_FILE, header=TRUE)
acci <- list()
