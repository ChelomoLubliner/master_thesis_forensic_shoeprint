...
# Load the main model for comparison (using config variables)
...
if (!is.null(main_model)) {
  ...
  if ("poids" %in% colnames(model_data)) {
    empty_model <- glmer(n_Acc ~ 1 + (1|shoe),
                         data = model_data,
                         family = binomial,
                         weights = model_data$poids)
  } else {
    # Create empty model without weights if poids column doesn't exist
    empty_model <- glmer(n_Acc ~ 1 + (1|shoe),
                         data = model_data,
                         family = binomial)
  }
...
model_comparison <- function(model_feat = MODEL_FEATURE) {
  ...
  file_name <- paste(SAVED_MODELS_DIR, model_feat, "_RELATIVE.rds", sep = "")
