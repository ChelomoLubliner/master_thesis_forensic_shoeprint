...
from globals import FOLDER, W, H, OLD_DATASET, SHARED_DATA_PATH, IMAGES_PATH


def get_image(entire_shoe, im_num,list_contour):
    img = np.nan
    if entire_shoe:
        img = img_as_ubyte(Image.open(os.path.join(IMAGES_PATH, 'Cleaned_Shoes', f'im_{im_num}.png')))
    else:
...
def save_img(img, snake, im_num):
    ...
    plt.close('all')
    img = Image.open(buf)
    img.save(os.path.join(IMAGES_PATH, 'Active_Contour', f'im_{im_num}.png'))
...
def active_contour_on_prototype():
    original_img = img_as_ubyte(Image.open(os.path.join(SHARED_DATA_PATH, 'old_freq_min_18.png')))
    init = init_snake()
    first_snake = active_contour(gaussian(original_img, 3, preserve_range=False), init, alpha=0.015, beta=10,gamma=0.001)
    comb_extremes_img = combined_snake_arr(first_snake, original_img)
    """Do second snake on comb_extremes_img"""
    ...
    final_img.save(os.path.join(SHARED_DATA_PATH, 'freq_min_18.png'))
    np.save(os.path.join(SHARED_DATA_PATH, 'freq_min_18.npy'), final_arr)
    return

def main():
    print(f"active_contour_snake")
    list_contour = np.load(os.path.join(SHARED_DATA_PATH, 'list_contour.npy'))
    snake_all = []
    ...
    np.save(os.path.join(SHARED_DATA_PATH, 'active-contour_all.npy'), snake_all)

    # Save as text file like in current version
