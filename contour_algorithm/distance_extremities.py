...
from globals import FOLDER, W, H, PROCESSING_DATA_PATH, LOCATIONS, LOCATIONS_HAS_HEADER, LOCATIONS_SEPARATOR, LOCATIONS_SCALE_FACTOR
import numpy as np
import pandas as pd
import math
...
def init_locations_new():
    if LOCATIONS_HAS_HEADER:
        locations = pd.read_csv(f'../Data/{LOCATIONS}', sep=LOCATIONS_SEPARATOR)
    else:
        locations = pd.read_csv(f'../Data/{LOCATIONS}', sep=LOCATIONS_SEPARATOR, header=None)
    ...
    """
    locations.to_csv(f'../shared_data/locations_new.csv', index=False)
...
def shortest_distance(shoe_df,shoe_num, locations_all, algo):
    ...
    locations_all.to_csv(f'../shared_data/locations_new.csv', index=False)
...
def main():
    ...
    init_locations_new()
    list_snake = list(np.load(f'{PROCESSING_DATA_PATH}active-contour_all.npy'))
    for i in tqdm(range(10)):#len(list_snake))
