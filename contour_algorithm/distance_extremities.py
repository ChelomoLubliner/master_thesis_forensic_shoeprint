...
import os
from globals import FOLDER, W, H, LOCATIONS, OLD_DATASET, DATA_PATH, SHARED_DATA_PATH, IMAGES_PATH
import numpy as np
import pandas as pd
import math
...
def init_locations_new():
    if OLD_DATASET :
        locations = pd.read_csv(os.path.join(DATA_PATH, LOCATIONS))
    else :
        locations = pd.read_csv(os.path.join(DATA_PATH, LOCATIONS), sep=' ', header=None)
...
def save_html(total_df, shoe_num,algo) :
    ...
    pyo.plot(fig, filename=os.path.join(IMAGES_PATH, f'{algo}_RAC_html', f'plot_cont_{shoe_num}.html'), auto_open=False)
...
def main():
    ...
    init_locations_new()
    list_snake = list(np.load(os.path.join(SHARED_DATA_PATH, 'active-contour_all.npy')))
