...
library(rgl)
library(fields)
library(survival)  # Needed for CMD_MODEL (clogit)
library(smoothie) 
...
# For NAIVE_MODEL, rand is already the intensity matrix - skip dataCC loading
if(MODEL_FEATURE != 'NAIVE_MODEL'){
  file_name <- paste(ROOT_PATH, "statistical_model/dataset/dataCC_distance.csv", sep = "")
  dataCC <- read.csv(file_name, header = TRUE)
} else {
  # Load allcont for NAIVE_MODEL (needed for masking)
...
img <- load.image(paste(ROOT_PATH, "images/cleaned_shoes/im_135.png", sep = "")) #prototype
img <- t(as.matrix(img))
...
if(MODEL_FEATURE == 'NAIVE_MODEL'){
  # For NAIVE_MODEL, rand is already the smoothed intensity matrix (from Naive())
  # Equivalent to: naive_smooth <- Naive(); image.plot(naive_smooth, axes=FALSE)
  naive_smooth <- as.matrix(rand)
  old_matrix <- naive_smooth
} else {
...
} else if(MODEL_FEATURE == 'EMPTY_MODEL'){
  # newdesignmat already has intercept column (rep(1,...))
  # No additional columns needed
} else if(MODEL_FEATURE == 'CMD_MODEL'){
  basx <- ns(dataCC$x,knots=knotsx)
  basy <- ns(dataCC$y,knots=knotsy)
  for(i in 1:length(predict(basy,1))) {
    for(j in 1:length(predict(basx,1))) {
      newdesignmat <-  cbind(newdesignmat,predict(basx, xy$x)[,j]*predict(basy, xy$y)[,i])  } }
  } else { stop("Invalid MODEL_FEATURE value")}
  ...
  # Extract coefficients - different for clogit vs glmer
  if(MODEL_FEATURE == 'CMD_MODEL'){
    # clogit models use coef() not fixef()
    # In conditional logistic regression, intercept cancels out, so we add 0
    model_coef <- c(0, coef(rand))
    pred.case_control <- newdesignmat%*%model_coef  # No offset for CML
  } else {
    # glmer models use fixef()
    model_coef <- fixef(rand)
    pred.case_control <- newdesignmat%*%model_coef+log(0.005) #log(0.005) is the offset
  }

  img_filled <- t(as.matrix(load.image(paste(ROOT_PATH,"statistical_model/dataset/new_filled_135.png", sep = ""))))
...
# For CMD_MODEL, optionally adjust mean to match other models (if needed)
# Previous code used: pred.cml.bin.case_control<-pred.cml.bin.case_control-m_1+m
# where m_1 is mean of CML predictions and m is target mean from another model
# Uncomment and set 'm' if you need this adjustment:
# if(MODEL_FEATURE == 'CMD_MODEL'){
#   m_1 <- mean(pred.case_control, na.rm=TRUE)
#   m <- 0  # Set this to the target mean from your reference model
#   pred.case_control <- pred.case_control - m_1 + m
# }

# Clean up variables (only remove those that exist)
if(MODEL_FEATURE != 'NAIVE_MODEL'){
  rm(newdesignmat, rand, dataCC)
  if(exists("bas_new_x")) rm(bas_new_x)
  if(exists("basy")) rm(basy)
  if(exists("basx")) rm(basx)
} else {
  rm(rand, allcont)
}
...
# For NAIVE_MODEL, old_matrix is already set above
...
file_name <- paste(ROOT_PATH, "statistical_model/model_images/",MODEL_FEATURE, "_SHOE.pdf", sep = "")
...
rotated_intens <- t(intens)[, nrow(intens):1]
image.plot(rotated_intens, axes = TRUE)
file_name <- paste(ROOT_PATH, "statistical_model/model_images/",MODEL_FEATURE, "_CONTOUR.pdf", sep = "")
pdf(file = file_name, height = 6, width = 6)
image.plot(rotated_intens, axes = TRUE)
abline(v = 0.5, col = "black", lty = 1, lwd = 1)
abline(h = 0.5, col = "black", lty = 1, lwd = 1)
...
new_contour <- t(as.matrix(load.image(paste(ROOT_PATH, "statistical_model/dataset/new_contour_171.png", sep = ""))))
entire_contour<-data.frame(x = xcoor[new_contour==1],y =ycoor[new_contour==1])# the data
distinct_y <- unique(entire_contour$y)
new_image <- t(as.matrix(load.image(paste(ROOT_PATH, "statistical_model/dataset/new_filled_",IMAGE_NUMBER,".png", sep = ""))))
```
...
new_img <- t(as.matrix(load.image(paste( ROOT_PATH, "images/cleaned_shoes/im_171.png", sep = ""))))
new_rotated[new_img==0] <- NA
im_rotated <- t(new_rotated)[, nrow(new_rotated):1]

image.plot(im_rotated, axes = TRUE)
file_name <- paste(ROOT_PATH, "statistical_model/model_images/", MODEL_FEATURE, "_", IMAGE_NUMBER, "_NEW_SHOE.pdf", sep = "")
pdf(file = file_name, height = 6, width = 6)
image.plot(im_rotated, axes = TRUE)
abline(v = 0.5, col = "black", lty = 1, lwd = 1)
abline(h = 0.5, col = "black", lty = 1, lwd = 1)
...
new_rotated <- old_matrix
im_rotated <- t(new_rotated)[, nrow(new_rotated):1]
file_name <- paste(ROOT_PATH, "statistical_model/model_images/", MODEL_FEATURE, "_", IMAGE_NUMBER, "_NEW_CONTOUR.pdf", sep = "")
pdf(file = file_name, height = 6, width = 6)
image.plot(im_rotated, axes = TRUE)
abline(v = 0.5, col = "black", lty = 1, lwd = 1)
abline(h = 0.5, col = "black", lty = 1, lwd = 1)
