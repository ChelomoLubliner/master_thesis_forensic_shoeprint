...
file_name <- paste(ROOT_PATH, "statistical_model/dataset/dataCC_distance_temp.csv", sep = "")
...
image <- t(as.matrix(load.image(paste(ROOT_PATH, "statistical_model/dataset/contour_prototype.png", sep = ""))))
xcoor <- t(matrix(rep((-col_shoe / 2 + 1:col_shoe) * rel_Y_cord / rel_col_shoe, row_shoe), col_shoe, row_shoe))
ycoor <- -matrix(rep((-row_shoe / 2 + 1:row_shoe) * rel_Y_cord / rel_col_shoe, col_shoe), row_shoe, col_shoe)
...
```{r }
#file_name <- paste(ROOT_PATH, "/statistical_model/dataset/contour_algorithm.csv", sep = "")
