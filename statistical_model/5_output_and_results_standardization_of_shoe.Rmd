...
model_images_dir <- paste(ROOT_PATH, CONTOUR_ALGORITHM, "/Model_Images/", sep = "")
...
file_name <- paste(MODEL_IMAGES_DIR, MODEL_FEATURE, "_SHOE.pdf", sep = "")
...
rotated_intens <- t(intens)[, nrow(intens):1]
image.plot(rotated_intens, axes = TRUE)
file_name <- paste(MODEL_IMAGES_DIR, MODEL_FEATURE, "_CONTOUR.pdf", sep = "")
pdf(file = file_name, height = 6, width = 6)
image.plot(rotated_intens, axes = TRUE)
abline(v = 0.5, col = "black", lty = 1, lwd = 1)
abline(h = 0.5, col = "black", lty = 1, lwd = 1)
...
new_contour <- t(as.matrix(load.image(paste(DATASET_DIR, "new_contour_171.png", sep = ""))))
...
new_image <- t(as.matrix(load.image(paste(DATASET_DIR, "new_filled_",IMAGE_NUMBER,".png", sep = ""))))
```
...
new_img_file <- paste(ROOT_PATH, "Images/Old_Shoes/Cleaned_Shoes/im_", IMAGE_NUMBER, ".png", sep = "")
if (file.exists(new_img_file)) {
  new_img <- t(as.matrix(load.image(new_img_file)))
} else {
  # Use the prototype image as fallback
  new_img <- img
  cat("Warning: Target image not found, using prototype image\n")
}
new_rotated[new_img==0] <- NA
coco_rotated <- t(new_rotated)[, nrow(new_rotated):1]

image.plot(coco_rotated, axes = TRUE)
file_name <- paste(MODEL_IMAGES_DIR, "NEW_X_NS_XY_",IMAGE_NUMBER, "_NEW_SHOE.pdf", sep = "")
pdf(file = file_name, height = 6, width = 6)
image.plot(coco_rotated, axes = TRUE)
abline(v = 0.5, col = "black", lty = 1, lwd = 1)
abline(h = 0.5, col = "black", lty = 1, lwd = 1)
...
new_rotated <- old_matrix
coco_rotated <- t(new_rotated)[, nrow(new_rotated):1]
file_name <- paste(MODEL_IMAGES_DIR, "NEW_X_NS_XY_",IMAGE_NUMBER, "_NEW_CONTOUR.pdf", sep = "")
pdf(file = file_name, height = 6, width = 6)
image.plot(coco_rotated, axes = TRUE)
abline(v = 0.5, col = "black", lty = 1, lwd = 1)
abline(h = 0.5, col = "black", lty = 1, lwd = 1)
