---
title: "Model Comparison: Naive, NEW_X_NS_XY, and CMD_MODEL"
...
# 2. CMD_MODEL (CML)
file_name <- paste(ROOT_PATH, "statistical_model/saved_models/CMD_MODEL.rds", sep = "")
...
pred.case_control_cml[t(img_filled)==0] <- NA #areas out of the contour (less than 8 shoes has contact surface in these pixels) are given NA
...
# Scale naive to match the range of scaled intensities
# Naive is a ratio (cumRAC/cumContact), while random and CML are intensities
# We scale naive proportionally to match the range of scaled intensities
range_scaled_intens <- range(c(intens_random_scaled, intens_cml_scaled), na.rm=TRUE)
range_naive <- range(naive_smooth[,cols], na.rm=TRUE)

# Scale naive to match the range of scaled intensities
if(diff(range_naive) > 0) {
  naive_scaled <- (naive_smooth[,cols] - range_naive[1]) / diff(range_naive) * diff(range_scaled_intens) + range_scaled_intens[1]
} else {
  naive_scaled <- naive_smooth[,cols]
}

com_3_est <- cbind(naive_scaled, intens_random_scaled, intens_cml_scaled)
image.plot(t(com_3_est[nrow(com_3_est):1,]), axes=FALSE, xlab='Naive                     NEW_X_NS_XY                     CMD_MODEL')
```

```{r}
pdf(file = paste(ROOT_PATH, "statistical_model/model_images/pixel_inten_JASAup.pdf", sep = ""), height=6, width=6)
image.plot(t(com_3_est[nrow(com_3_est):1,]), axes=FALSE, xlab='Naive                     NEW_X_NS_XY                     CMD_MODEL')
dev.off()
```
...
naive_display <- t(naive_scaled[nrow(naive_scaled):1,])
image.plot(naive_display, axes=FALSE, main='Naive (scaled)')
```
...
image.plot(intens_cml_display, axes=FALSE, main='CMD_MODEL (scaled)')
```
